...
## Task 25 — Overhaul app_web.py visual design for coherent dark theme: COMPLETE, verified
...
**Implementation:** Created a coherent dark theme design system using CSS variables with the existing Color Hunt palette (e73f1e, fb6c00, 0f9b63, 7ffdd9) plus WCAG-compliant light variants (ff6b4a, ff9d4a, 2dd494) for badges on dark backgrounds. Applied consistent dark palette across all UI surfaces: sidebar, chat messages, tabs, prediction cards, badges (confidence/consistency), quality/finding tables, cleaner section, buttons, and replaced all Streamlit default element styling leaks. Standardized border-radius to 12px (cards, chat, input) and 20px (badges), and consolidated gray colors to 3 levels (primary #f0f0f0, secondary #b8c5d0, tertiary #8a9bb0). Removed Google Fonts import and replaced with system font stack for offline compliance.

**Contrast verification:** Programmatically computed WCAG contrast ratios for all 26 text/background color pairs. All pairs pass AA with margin (lowest ratio: 5.93:1 against 4.5:1 minimum for normal text). Confirmed no transition/transform/:hover CSS rules were reintroduced (only text-transform: uppercase remains, which is styling, not animation).

**Test verification:** Full test suite green (166 passed, 0 failed, 0 skipped) — identical to baseline, confirming no behavioral/logic changes occurred. tests/test_app_web_data_shapes.py and tests/test_app_web_cleaning_adoption.py both pass, confirming app_web.py source text assertions remain valid. Smoke-tested both tabs (RAG Chat empty state, Predictor empty state) render without Python errors.

**Design decisions:** Maintained dark theme to match other UI elements in the project. Used light variants of palette colors for badges to ensure WCAG AA compliance on dark backgrounds (original palette colors failed contrast on dark backgrounds). The 4th palette color (#7ffdd9) remains unused as planned — no suitable use case emerged during the coherent application phase; future work can introduce it where semantically appropriate.
